...
class Puzzle14Part2 extends Command
{
    protected static $defaultName = 'puzzle-14-part-2';

    private $recipies = [];
    private $stock = ['ORE' => 1000000000000, 'FUEL' => 0];
    /** @var OutputInterface */
    private $output;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        while (true) {
            $i++;
            $this->stock = ['ORE' => 1000000000000, 'FUEL' => 0];
            $this->take('FUEL', $i);

            $used = 1000000000000 - $this->stock['ORE'];
            $output->writeln("$used ore was required to produce $i FUEL");
        }
    ...
    {
        $this->output->writeln("Taking $amount $type", OutputInterface::VERBOSITY_VERY_VERBOSE);
        ...
        $this->output->writeln("Stock level for $type decreased to {$this->stock[$type]} (-$amount)", OutputInterface::VERBOSITY_VERY_VERBOSE);
    }

    private function produce(string $type, int $amount)
    {
        if (! array_key_exists($type, $this->recipies)) {
            throw new \Exception("Cant produce $type");
        }

        $recipie = $this->recipies[$type];
        $this->output->writeln("Producing $amount $type", OutputInterface::VERBOSITY_VERY_VERBOSE);
        ...
        $this->output->writeln("Stock level for $type increased to {$this->stock[$type]} (+{$productionAmount})", OutputInterface::VERBOSITY_VERY_VERBOSE);
    }
