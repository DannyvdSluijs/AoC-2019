...
class Puzzle11Part2 extends Command
{
    protected static $defaultName = 'puzzle-11-part-2';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->paintPanel($robotLocation, self::WHITE);
        ...
        foreach ($this->grid as $row) {
            $line = str_replace([self::BLACK], ' ', implode($row));
            $output->writeln($line);
        }
    ...
    {
        if (! array_key_exists($location->y, $this->grid)) {
            return self::BLACK;
        }
        if (! array_key_exists($location->x, $this->grid[$location->y])) {
            return self::BLACK;
        }
