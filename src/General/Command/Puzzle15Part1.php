    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $callback = function(SplQueue $in, SplQueue $out) use (&$position, &$path) {
            ...
            if ($result === self::HIT_WALL) {
                ...
                $this->output->write(sprintf("Hit a wall at % 3d,% 3d;", $wall->x, $wall->y));
                $this->output->writeln('Path: ' . $this->dumpPath(clone $path));
            ...
            if ($result === self::MOVED) {
                ...
                $this->output->write(sprintf('Found path at % 3d,% 3d ', $position->x, $position->y));
                $this->output->writeln('Path: ' . $this->dumpPath(clone $path));
                $backTracking && $this->output->writeln('');
            }
            ...
            while (true) {
                if (! $this->isPositionKownInGrid($try)) {
                    if ($backTracking) {
                        $this->output->writeln('No longer tracking back in path.');
                        $backTracking = false;
                    }
                    break;
                }
                ...
                if ($attempts >= 3) {
                    ...
                    $this->output->writeln('Tracking back in path.');
                    $backTracking = true;
                    break;
                }
            }
            ...
            $this->output->write(sprintf("From pos % 3d,% 3d moving $dir: ", $position->x, $position->y));
