...
{
    protected static $defaultName = 'puzzle-3';

    private $grid = [];
    private $manhattanDistances = [];
    private $sumOfWireSteps = [];

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        [$line1, $line2] = explode(PHP_EOL, $this->getPuzzleInput());

        $this->initialiseGrid(12000, 12000);
        $output->writeln('Initialised grid');
        $this->drawLine($line1, 1);
        $output->writeln('Line 1 finished');
        $this->drawLine($line2, 2, true);
        $output->writeln('Line 2 finished');

        $shortestDistance = min(...$this->manhattanDistances);
        $output->writeln("The shortest manhattan distance is $shortestDistance");

        $shortestWireSteps = min(...$this->sumOfWireSteps);
        $output->writeln("The shortest wire steps is $shortestWireSteps");
...
//        return 'R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51' . PHP_EOL .'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7'; /* Answer 135 */
    ...
    private function initialiseGrid(int $height, int $width): void
    {
        $row = [];
    ...
    private function drawLine(string $instructions, int $id, bool $markCollission = false): void
    {
        $x = 0;
        $y = 0;
        $this->grid[$x][$y] = 'o';
        ...
        foreach (explode(',', $instructions) as $instruction) {
            $direction = ucfirst(substr($instruction, 0, 1));
            ...
            for ($l = 0; $l < $length; $l++) {
                ...
                if ($markCollission && is_object($this->grid[$x][$y]) && $this->grid[$x][$y]->id !== $id) {
                    $this->manhattanDistances[] = abs($x) + abs($y);
                    $this->sumOfWireSteps[] = $this->grid[$x][$y]->steps + ++$steps;
                } else {
                    $this->grid[$x][$y] = (object) ['id' => $id, 'steps' => ++$steps];
                }
            }
        }
    }
